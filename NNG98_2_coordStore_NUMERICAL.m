...
function [PE_tot,DPEDxyz] = NNG98_2_coordStore_NUMERICAL(coord,total,numMov,numPeriph,numBound,movAtom,periphAtom,boundAtom)
...
point7=[0.01;0.02;0.03;-0.01;-0.02;-0.03];

for i=1:numMov
    iMov=movAtom(i);
    
    %####################################START INCREMENT IN X-DIRECTION#################################
    rij = 0;	DrijDxi = 0;	theta = 0;	inDimension = 0;	in = 0;	chainruleFactor = 0;	DPEDin = 0;
    
    for countPoint=1:6
        x(iMov) = x(iMov) + point7(countPoint);
        ...
        for j=2:list(iMov,1)+1
            ...
            %@*@*@*@*@*@*FOLLOWING CODE FOR ANALYTICAL DIFFERENTIATION IS COMMENTED BECAUSE WE ARE CALCULATING  FORCE i.e. DERIVATIVE NUMERICALLY
            % 		DrijDxi(j-1) = (x(iMov) - x(jBond))./rij(j-1);		DrijDyi(j-1) = (y(iMov) - y(jBond))./rij(j-1);		DrijDzi(j-1) = (z(iMov) - z(jBond))./rij(j-1);
            
            % 		chainruleFactor(inDimension,1) = DrijDxi(j-1);		chainruleFactor(inDimension,2) = DrijDyi(j-1);		chainruleFactor(inDimension,3) = DrijDzi(j-1);
            %@*@*@*@*@*@*ABOVE CODE FOR ANALYTICAL DIFFERENTIATION IS COMMENTED BECAUSE WE ARE CALCULATING  FORCE i.e. DERIVATIVE NUMERICALLY
        end	
        ...
        for k = 4:list(iMov,1)+1 % START calculate the dihedral angle phi, which is a 4-body term
            ...
            if((k3Body == iMov) | (k3Body == jBond) | (k4 == iMov) | (k4 == jBond)), continue;		end
            %*************************************************************************************************************************************************
        ...
        %         % if(iMov==14 | iMov==137 | iMov==164 | iMov==191 | iMov==218)
        %         if(list(iMov,1) == 4)
    ...
    %####################################END INCREMENT IN X-DIRECTION###################################
    
    %####################################START INCREMENT IN Y-DIRECTION#################################
    
    rij = 0;	DrijDxi = 0;	theta = 0;	inDimension = 0;	in = 0;	chainruleFactor = 0;	DPEDin = 0;
    
    for countPoint=1:6
        y(iMov) = y(iMov) + point7(countPoint);
    ...
    %###########################END OF INCREMENT IN Y-DIRECTION#####################################
    
    %####################################START INCREMENT IN Z-DIRECTION#################################
    
    rij = 0;	DrijDxi = 0;	theta = 0;	inDimension = 0;	in = 0;	chainruleFactor = 0;	DPEDin = 0;
    
    for countPoint=1:6
        z(iMov) = z(iMov) + point7(countPoint);
    ...
    %###########################END OF INCREMENT IN Z-DIRECTION #####################################
