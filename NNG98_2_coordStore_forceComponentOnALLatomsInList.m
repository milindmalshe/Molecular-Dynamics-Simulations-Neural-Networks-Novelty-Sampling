...
function [PE_tot,DPEDxyz] = NNG98_2_coordStore_forceComponentOnALLatomsInList(coord,total,numMov,numPeriph,numBound,movAtom,periphAtom,boundAtom)
...
for i=1:numMov
    ...
    for k=3:list(iMov,1)+1 % START calculate bond angle theta, which is a 3-body term
        ...
        DthetaDy3 = DthetaDrij.*chainruleFactor(j-1,2,k) + DthetaDrjk*(-DrjkDyj);
        DthetaDz3 = DthetaDrij.*chainruleFactor(j-1,3,k) + DthetaDrjk*(-DrjkDzj);
